...
absl::Status DecisionTreeCoreTrain(
  ...
  switch (dt_config.growing_strategy_case()) {
    case proto::DecisionTreeTrainingConfig::kGrowingStrategyLocal: {
      ...
      // ADDED BY JY
      #if defined(BFS_ONLY) 
        std::cout << "BFS_ONLY Defined. " << std::endl;
      ...
      #else 
        std::cout << "BFS_ONLY Not Defined. " << std::endl;
...
absl::Status GrowTreeLocalBFS(
  ...
  int* d_pos_examples_persist = nullptr;
  int* d_neg_examples_persist = nullptr;
  ...
  CUDA_CHECK(cudaMalloc(&d_pos_examples_persist, static_cast<size_t>(num_rows) * sizeof(int)));
  CUDA_CHECK(cudaMalloc(&d_neg_examples_persist, static_cast<size_t>(num_rows) * sizeof(int)));
  ...
  bool has_device_split_results = false;
  std::vector<ChildMapping> compact_children;
  ...
  while (!node_queue.empty()) {
    ...
    // std::cout << "Time taken for depth " << current_depth << " processing: " << elapsed_time_depth.count() << " ms." << std::endl;
      ...
      if (dt_config.has_sparse_oblique_split() && depth_batch.size() >= 1) {
        ...
        std::cerr << "  D" << current_depth << " FlattenSelectedExamples start, batch_size=" << depth_batch.size() << "\n";
        auto flat_sel_result =
            internal::FlattenSelectedExamples(depth_batch);
        std::cerr << "  D" << current_depth << " FlattenSelectedExamples done\n";
        ...
        if (has_device_split_results) {
          std::cerr << "  D" << current_depth << " CompactSplitResults: "
                    << compact_children.size() << " children\n";
          // Fill in dst_off from this depth's node_row_offsets
          for (int n = 0; n < static_cast<int>(compact_children.size()); ++n) {
            compact_children[n].dst_off = node_row_offsets[n];
          }
          CompactSplitResults_wrap(
              d_pos_examples_persist, d_neg_examples_persist,
              d_selected_examples, compact_children);
          std::cerr << "  D" << current_depth << " CompactSplitResults done\n";
        } else {
          // First depth: standard flatten + H2D
          for (int n = 0; n < num_nodes; ++n) {
        ...
        // d_selected_examples already populated by CompactSplitResults_wrap when has_device_split_results
        ...
        if (combined) // BFS Only and GPU Apply Projection on Evaluate Projection
        {
          ...
          // Build node_sizes for the no-D2H function
          std::vector<int> node_sizes(num_nodes);
          for (int n = 0; n < num_nodes; ++n) {
            node_sizes[n] = static_cast<int>(sel_spans[n].size());
          }

          ASSIGN_OR_RETURN(auto split_result,
            SplitExamplesInPlaceKernel_all_depth_no_d2h(
                  d_col_add_projected,
                  d_selected_examples,
                  d_best_proj_per_node,
                  d_best_threshold_per_node,
                  ...
                  d_labels,
                  num_classes,
                  d_pos_examples_persist,
                  d_neg_examples_persist));

          auto& pos_class_counts = split_result.pos_class_counts;
          auto& neg_class_counts = split_result.neg_class_counts;
          auto& total_pos_per_node = split_result.total_pos_per_node;

          // Build example_splits from span sizes (no data needed under BFS_ONLY)
          using ExSplitRB = yggdrasil_decision_forests::model::decision_tree::ExampleSplitRollingBuffer;
          std::vector<ExSplitRB> example_splits(num_nodes);
          for (int n = 0; n < num_nodes; ++n) {
            int tp = total_pos_per_node[n];
            int inact_sz = static_cast<int>(inactive_spans[n].size());
            int sel_sz = static_cast<int>(sel_spans[n].size());
            std::cerr << "  D" << current_depth << " node=" << n
                      << " tp=" << tp << " inact_sz=" << inact_sz
                      << " sel_sz=" << sel_sz << "\n";
            int tp_i = std::min(tp, inact_sz);
            int tp_s = std::min(tp, sel_sz);
            example_splits[n].positive_examples = {
                .active = inactive_spans[n].subspan(0, tp_i),
                .inactive = sel_spans[n].subspan(0, tp_s)};
            example_splits[n].negative_examples = {
                .active = inactive_spans[n].subspan(tp_i),
                .inactive = sel_spans[n].subspan(tp_s)};
          }

          has_device_split_results = true;
        ...
        // --- [8] cudaFree metadata buffers ---
          ...
          std::cerr << "  D" << current_depth << " NodeTrain start\n";
          ...
          // Build compact mapping for next depth's CompactSplitResults kernel.
          // NodeTrain pushes neg child first, then pos child, for each
          // node that splits with both children non-empty.
          compact_children.clear();
          for (int n = 0; n < num_nodes; ++n) {
            if (!node_has_split[n]) continue;
            int tp = total_pos_per_node[n];
            int tn = static_cast<int>(sel_spans[n].size()) - tp;
            if (tp == 0 || tn == 0) continue;
            int src = node_row_offsets[n];
            compact_children.push_back({src, 0, tn, 0});  // neg child
            compact_children.push_back({src, 0, tp, 1});  // pos child
          }
    ...
    {
      // BFS all CPU path
      std::cerr << "BFS default path: processing " << depth_batch.size()
                << " nodes with NodeTrain" << std::endl;
      for (size_t i = 0; i < depth_batch.size(); ++i) {
        auto& nae = depth_batch[i];
        std::cerr << "  NodeTrain node " << i
                  << " examples=" << nae.selected_examples.size()
                  << " depth=" << nae.depth << std::endl;
        RETURN_IF_ERROR(NodeTrain(train_dataset, config, config_link, dt_config,
                                  deployment, weights, internal_config, random,
                                  cache, std::move(nae), node_queue));
        std::cerr << "  NodeTrain node " << i << " done" << std::endl;
      }
    }
  ...
  if (combined) {
    ...
    internal::PrintAndResetDfsTimers();
    PrintAndResetSplitAllDepthTimers();
    if (total_node_train_count > 0) {
      std::cerr << "NodeTrain TOTAL setup=" << total_node_setup_ms << "ms"
                << " find_best=" << total_find_best_ms << "ms"
                << " split_examples=" << total_split_examples_ms << "ms"
  ...
  cudaFree(d_pos_examples_persist);
  cudaFree(d_neg_examples_persist);

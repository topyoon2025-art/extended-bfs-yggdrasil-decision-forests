...
// --- No-D2H version: keeps pos/neg on device, returns counts only ---
                ...
                int* d_neg_examples)
{
    ...
    // Small D2H: gather total_pos per node + class counts
    int* d_total_pos;
    CUDA_CHECK(cudaMalloc(&d_total_pos, num_nodes * sizeof(int)));
    {
      int gather_blocks = (num_nodes + 255) / 256;
    ...
    SplitAllDepthResult result;
    result.total_pos_per_node.resize(num_nodes);
    result.total_neg_per_node.resize(num_nodes);
    CUDA_CHECK(cudaMemcpy(result.total_pos_per_node.data(), d_total_pos,
                          num_nodes * sizeof(int), cudaMemcpyDeviceToHost));
    cudaFree(d_total_pos);

    for (int n = 0; n < num_nodes; ++n) {
        result.total_neg_per_node[n] = node_sizes[n] - result.total_pos_per_node[n];
    }
    ...
    result.pos_class_counts.resize(num_nodes);
    result.neg_class_counts.resize(num_nodes);
    for (int n = 0; n < num_nodes; ++n) {
      result.pos_class_counts[n].assign(
          h_class_counts.begin() + n * 2 * num_classes + 1 * num_classes,
          h_class_counts.begin() + n * 2 * num_classes + 2 * num_classes);
      result.neg_class_counts[n].assign(
          h_class_counts.begin() + n * 2 * num_classes + 0 * num_classes,
          h_class_counts.begin() + n * 2 * num_classes + 1 * num_classes);
    }
    ...
    auto ms = [](auto a, auto b) { return std::chrono::duration<double,std::milli>(b - a).count(); };
    std::cerr << "  SplitAllDepth(no-d2h) microtiming:"
              << " malloc=" << ms(t0, t1)
              << " flags_kernel=" << ms(t1, t2)
              << " prefix_sum=" << ms(t2, t3)
              << " scatter_kernel=" << ms(t3, t4)
    ...
    s_sad_prefix      += ms(t2, t3);
    s_sad_scatter     += ms(t3, t4);
    s_sad_d2h_counts  += ms(t4, t5);
    s_sad_free        += ms(t5, t6);

    return result;
}

// Compact kernel: rearrange previous depth's d_pos/d_neg into d_selected_examples
// for the next depth. Each child node maps to a region of the parent's pos or neg.
__global__ void CompactSplitResultsKernel(
    const int* __restrict__ d_pos_examples,
    const int* __restrict__ d_neg_examples,
    int* __restrict__ d_selected_examples,
    const ChildMapping* __restrict__ d_children,
    int num_children)
{
    int child = blockIdx.y;
    if (child >= num_children) return;

    int count = d_children[child].count;
    int r = blockIdx.x * blockDim.x + threadIdx.x;
    if (r >= count) return;

    const int* src = d_children[child].is_pos ? d_pos_examples : d_neg_examples;
    d_selected_examples[d_children[child].dst_off + r] = src[d_children[child].src_off + r];
}
...
{
    const int num_children = static_cast<int>(children.size());
    if (num_children == 0) return;

    int max_count = 0;
    for (const auto& c : children) max_count = std::max(max_count, c.count);

    constexpr int BLOCK = 256;
    int blocks_x = (max_count + BLOCK - 1) / BLOCK;
    dim3 grid(blocks_x, num_children);

    ChildMapping* d_children;
    CUDA_CHECK(cudaMalloc(&d_children, num_children * sizeof(ChildMapping)));
    CUDA_CHECK(cudaMemcpy(d_children, children.data(),
                          num_children * sizeof(ChildMapping), cudaMemcpyHostToDevice));

    CompactSplitResultsKernel<<<grid, BLOCK>>>(
        d_pos_examples, d_neg_examples, d_selected_examples,
        d_children, num_children);
    CUDA_CHECK(cudaDeviceSynchronize());
    CUDA_CHECK(cudaGetLastError());
